              ...
              <div className="flex flex-col gap-4 px-2 pb-4">
                {differentCollection &&
                  differentCollection?.map((result) => (
                    ...
                    >
                      <div className="w-full h-[173px] bg-white flex gap-4 rounded-2xl">
                        <img
                          className="w-[169px] h-full rounded-l-2xl"
                          src={`http://localhost:8080/images/${result?.image}`}
                          alt={result?.title}
                        />
                        <div className="flex flex-col gap-12 py-2">
                          <span>{result?.title}</span>
                          <span>{result?.view} Lượt xem</span>
                        </div>
                      </div>
                    </Link>
                  ))}
